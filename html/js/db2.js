 ...
 */
window.database.getUsers = () => {
    return window.database.vars.users;
};
...
window.database.addUser = async (firstname, lastname) => {
    ...
    if (result && result.length > window.database.vars.users.length) {
        window.database.vars.users.push({
            FirstName: firstname,
            LastName: lastname,
        });
        return true;
 ...
 */
window.database.getQuestions = async (
    package,
    category,
    subcategory,
    options
) => {
    var questions = await window.db.getAllQuestions(
        category,
        subcategory,
        options
    );
 ...
 */
window.database.addScore = async (
    package,
    category,
    subcategory,
    options,
