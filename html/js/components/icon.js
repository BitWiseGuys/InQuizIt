...
Vue.component("vIcon", {
    ...
    template: `<a  :class="'bi bi-' + icon + ' ' + shape + ' ' + (disabled ? 'disabled' : '') + (options && options['class'] ? ' ' + options['class'] : '') + ((options && options['sticky'] && pressed) ? ' icon-pressed' : '')" @click="$emit('click',$event);"></a>`,
    data() {
        return {
            shape : "icon-circle",
        }
