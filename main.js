...
app.whenReady().then(() => {
    ipcMain.handle("openPackagesDir", async () => {
        return dialog.showOpenDialog(BrowserWindow.getFocusedWindow(), {
            ...
            filters: [{ name: "Packages", extensions: ["db"] }],
        });
    });
...
ipcMain.handle(
    "newQuestion",
    async (event, Category, Name, Options, Type, Question) => {
        const res = await dbMngr.newQuestion(
            Category,
            Name,
            Options,
            Type,
...
ipcMain.handle(
    "newAnswer",
    async (event, Category, Name, Options, Type, Question, Ans) => {
        const res = await dbMngr.newAnswer(
            Category,
            Name,
            Options,
            Type,
...
ipcMain.handle(
    "getQuestionSet",
    async (event, setCategory, setName, setOptions) => {
        const res = await dbMngr.getQuestionSet(
            setCategory,
            setName,
            setOptions
        );
...
ipcMain.handle(
    "getAllQuestions",
    async (event, setCategory, setName, setOptions) => {
        const res = await dbMngr.getAllQuestions(
            setCategory,
            setName,
            setOptions
        );
...
ipcMain.handle(
    ...
    async (
        ...
        const res = await dbMngr.getAllAnswers(
            setCategory,
            setName,
            setOptions,
            questionContent,
...
ipcMain.handle(
    ...
    async (
        ...
        const res = await dbMngr.deleteQuestion(
            setCategory,
            setName,
            setOptions,
            questionContent,
...
ipcMain.handle(
    "delQuestionSet",
    async (event, setCategory, setName, setOptions) => {
        const res = await dbMngr.deleteQuestionSet(
            setCategory,
            setName,
            setOptions
        );
